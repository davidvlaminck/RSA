...
class Report0069(BaseReport):
    def init_report(self) -> None:
        self.report = LegacyHistoryReport(name='report0069',
                                          title="Detailrapport invulgraad legacy installaties EW Limburg",
                                          spreadsheet_id='1xPhHRqYbD24ToUADqokcqm5HlDHMtHxBRoyhtk4LlZ0',
                                          datasource='PostGIS',
                                          frequency=7,
                                          persistent_column='K')
...
records AS ( 
	...
	WHERE assets.actief = TRUE AND assettypes.uri NOT LIKE 'https://grp%' AND assettypes.uri NOT LIKE 'https://wegen%' AND toezichtgroepen.referentie = 'AWV_EW_LB' ) 
SELECT *
FROM records
