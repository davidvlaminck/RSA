...
class Report0067(BaseReport):
    def init_report(self) -> None:
        self.report = LegacyHistoryReport(name='report0067',
                                          title="Detailrapport invulgraad legacy installaties TOV",
                                          spreadsheet_id='1T1lgCb8FMmfUaYo_-G0oQWPgpt7MoKOACYC2yqQiOX4',
                                          datasource='PostGIS',
                                          frequency=7,
                                          persistent_column='K')
...
records AS ( 
	...
	WHERE assets.actief = TRUE AND assettypes.uri NOT LIKE 'https://grp%' AND assettypes.uri NOT LIKE 'https://wegen%' AND toezichtgroepen.referentie = 'TOV' ) 
SELECT *
FROM records
