...
class Report0070(BaseReport):
    def init_report(self) -> None:
        self.report = LegacyHistoryReport(name='report0070',
                                          title="Detailrapport invulgraad legacy installaties EW Oost-Vlaanderen",
                                          spreadsheet_id='1dzIq6pBmKQSzk-KSHtx2NIjwBnV3jb8OuPxSf46Iw0Q',
                                          datasource='PostGIS',
                                          frequency=7,
                                          persistent_column='K',
                                          excel_filename='[RSA] Ontbrekende gegevens installaties V&W-WO.xlsx')
...
records AS ( 
	...
	WHERE assets.actief = TRUE AND assettypes.uri NOT LIKE 'https://grp%' AND assettypes.uri NOT LIKE 'https://wegen%' AND toezichtgroepen.referentie = 'AWV_EW_OV' ) 
SELECT *
FROM records
