...
class Report0071(BaseReport):
    def init_report(self) -> None:
        self.report = LegacyHistoryReport(name='report0071',
                                          title="Detailrapport invulgraad legacy installaties EW Vlaams-Brabant",
                                          spreadsheet_id='1l3fkoacvQ_ka5QpErK6CwS9ZQo8U-EgpQksS4iPuQW8',
                                          datasource='PostGIS',
                                          frequency=7,
                                          persistent_column='K',
                                          excel_filename='[RSA] Ontbrekende gegevens installaties V&W-WVB.xlsx')
...
records AS ( 
	...
	WHERE assets.actief = TRUE AND assettypes.uri NOT LIKE 'https://grp%' AND assettypes.uri NOT LIKE 'https://wegen%' AND toezichtgroepen.referentie = 'AWV_EW_VB' ) 
SELECT *
FROM records
